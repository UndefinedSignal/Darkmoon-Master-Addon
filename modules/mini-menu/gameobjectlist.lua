function DMA:ProcessDMAGobjectList(chatstring)
	...
	table.insert(DMA.GOB.ObjectList, {DMA.GOB.Counter, entryid, entryname});
	DMA.GOB.Counter = DMA.GOB.Counter + 1;
end

function DMA:GenerateObjectsScrollMenu()
  FauxScrollFrame_Update(DMA_MiniMenuContentGameobjectObjectSceneScrollBar,#DMA.GOB.ObjectList,13,32);
...
function DMA:ObjectsScrollBar_Update()
    if DMA.GOB.ObjectListInitialized then
        for line=1,#DMA.lineplusoffset do
            ...
            if lineplusoffset <= #DMA.GOB.ObjectList then
                _G["DMA_MiniMenuContentGameobjectObjectSceneGOB"..line.."Number"]:SetText("|cFFFD8A00" .. DMA.GOB.ObjectList[lineplusoffset][1] .. "|r");
                _G["DMA_MiniMenuContentGameobjectObjectSceneGOB"..line.."Name"]:SetText(DMA.GOB.ObjectList[lineplusoffset][3]);
