...
underlineFrame:SetScript("OnShow", function(self) 	-- change sizing on the fly to catch runtime uiscale changes
    underlineFrame.tx:SetPoint("TOPLEFT", -1, -2/self:GetEffectiveScale())
    underlineFrame.tx:SetPoint("RIGHT", 1,0)
    underlineFrame.tx:SetHeight(0.6 / self:GetEffectiveScale());
end)
...
local function showGameTooltip(self)
	if self.tooltipTitle or self.tooltipText then
		...
		if self.tooltipTitle then
			if SharedMedia and SharedMedia:IsValid("font", self.tooltipTitle) then
				font = SharedMedia:Fetch("font", self.tooltipTitle)
			end
			if disabled then
			...
			if self.tooltipText then
				if not font and SharedMedia and SharedMedia:IsValid("font", self.tooltipText) then
					font = SharedMedia:Fetch("font", self.tooltipText)
				end
				if disabled then
			...
			if SharedMedia and SharedMedia:IsValid("font", self.tooltipText) then
				font = SharedMedia:Fetch("font", self.tooltipText)
			end
			if disabled then
...
local function AcquireButton(self, level)
	...
	if #buttons == 0 then
		...
		button:SetScript("OnEnter", function()
			...
			self:Close(self.level.num + 1)
			if not self.disabled then
				if self.secure then
					secureFrame:Activate(self)
				elseif self.hasSlider then
					OpenSlider(self, self)
				elseif self.hasEditBox then
					OpenEditBox(self, self)
				elseif self.hasArrow then
					Open(self, self, nil, self.level.num + 1, self.value)
				end
			end
			if not self.level then -- button reclaimed
				return
			end
			StopCounting(self, self.level.num + 1)
			if not self.disabled then
				highlight:Show()
		...
		button:SetScript("OnLeave", function()
			...
			if self.level then
				StartCounting(self, self.level.num)
			end
			GameTooltip:Hide()
		end)
		local first = true
...
function OpenEditBox(self, parent)
	if not editBoxFrame then
		...
		editBoxFrame:SetScript("OnKeyDown", function(self, a1)
		...
		editBoxFrame:SetScript("OnMouseWheel", function(t, a1)
			...
			return editBoxFrame:GetScript("OnKeyDown")(t or self, arg1)
		end)
