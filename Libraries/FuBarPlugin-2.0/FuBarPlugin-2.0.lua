...
function FuBarPlugin:CreateBasicPluginFrame(name)
	...
	function frame_OnClick()
			if self:IsMinimapAttached() and dragged then return end
	...
	if not frame_OnReceiveDrag then
		function frame_OnReceiveDrag()
			if (self:IsMinimapAttached() and not dragged) and type(self.OnReceiveDrag) == "function" then
				self:OnReceiveDrag()
			end
		end
	end
...
function FuBarPlugin:CreatePluginChildFrame(frameType, name, parent)
	...
	if not child_OnMouseUp then
		function child_OnMouseUp(...)
			if self.frame:HasScript("OnMouseUp") and self.frame:GetScript("OnMouseUp") then
	...
	if not child_OnReceiveDrag then
		function child_OnReceiveDrag(self)
			if self.frame:HasScript("OnReceiveDrag") and self.frame:GetScript("OnReceiveDrag") then
	...
	if not plugin.minimapFrame then
		...
		if not minimap_OnMouseDown then
			function minimap_OnMouseDown()
				...
				if self.OnClick or self.OnMouseDown or self.OnMouseUp or self.OnDoubleClick then
					if self.minimapIcon:GetTexture():sub(1, 16) == "Interface\\Icons\\" then
						self.minimapIcon:SetTexCoord(0.14, 0.86, 0.14, 0.86)
					else
						self.minimapIcon:SetTexCoord(0.1, 0.9, 0.1, 0.9)
					end
				end
			end
		end
		...
		if not minimap_OnMouseUp then
			function minimap_OnMouseUp()
				if not self.dragged and type(self.OnMouseUp) == "function" then
					self:OnMouseUp(arg1)
				end
				if self.minimapIcon:GetTexture():sub(1, 16) == "Interface\\Icons\\" then
					self.minimapIcon:SetTexCoord(0.05, 0.95, 0.05, 0.95)
...
function MinimapContainer.OnDragStart()
	dragged = true
	self:LockHighlight()
	self:SetScript("OnUpdate", MinimapContainer.OnUpdate)
--	if self.minimapIcon:GetTexture():sub(1, 16) == "Interface\\Icons\\" then
--		self.minimapIcon:SetTexCoord(0.05, 0.95, 0.05, 0.95)
--	else
--		self.minimapIcon:SetTexCoord(0, 1, 0, 1)
...
function MinimapContainer.OnUpdate()
	if not IsAltKeyDown() then
		...
		if self.db then
			...
			self.db.profile.minimapPositionX = nil
			self.db.profile.minimapPositionY = nil
			self.db.profile.minimapPositionWild = nil
		else
			self.minimapPosition = position
			self.minimapPositionX = nil
			self.minimapPositionY = nil
			self.minimapPositionWild = nil
		end
	else
		local px, py = GetCursorPosition()
		local scale = UIParent:GetEffectiveScale()
		px, py = px / scale, py / scale
		if self.db then
			self.db.profile.minimapPositionX = px
			self.db.profile.minimapPositionY = py
			self.db.profile.minimapPosition = nil
			self.db.profile.minimapPositionWild = true
		else
			self.minimapPositionX = px
			self.minimapPositionY = py
			self.minimapPosition = nil
			self.minimapPositionWild = true
		end
	end
	MinimapContainer:ReadjustLocation(self)
end
