...
local function button_OnEnter(self, ...)
	if type(self.self:GetScript("OnEnter")) == "function" then
		self.self:GetScript("OnEnter")(self.self, ...)
	end
	self.highlight:Show()
	if self.onEnterFunc then
		local success, ret = pcall(self.onEnterFunc, argunpack(self, 'onEnterArg'))
		if not success then
			geterrorhandler()(ret)
		end
	end
...
local function button_OnLeave(self, ...)
	if type(self.self:GetScript("OnLeave")) == "function" then
		self.self:GetScript("OnLeave")(self.self, ...)
	end
	self.highlight:Hide()
	if self.onLeaveFunc then
		local success, ret = pcall(self.onLeaveFunc, argunpack(self, 'onLeaveArg'))
		if not success then
			geterrorhandler()(ret)
		end
	end
...
local function button_OnClick(self, arg1, ...)
	if self.self:HasScript("OnClick") and type(self.self:GetScript("OnClick")) == "function" then
		self.self:GetScript("OnClick")(self.self, arg1, ...)
	end
	...
	elseif arg1 == "LeftButton" then
		if self.self.preventClick == nil or GetTime() > self.self.preventClick and GetTime() < lastMouseDown + 0.5 then
			self.self.preventClick = nil
			self.self.updating = true
			self.self.preventRefresh = true
			local success, ret = pcall(self.func, argunpack(self, 'arg'))
			if not success then
				geterrorhandler()(ret)
			end
			if self.self and self.self.registration then
				self.self.preventRefresh = false
				self.self:children()
				self.self.updating = false
			end
		end
	end
end
button_OnClick = wrap(button_OnClick, "button_OnClick")
local function button_OnMouseUp(self, arg1, ...)
	if self.self:HasScript("OnMouseUp") and type(self.self:GetScript("OnMouseUp")) == "function" then
		self.self:GetScript("OnMouseUp")(self.self, arg1, ...)
	end
	if arg1 ~= "RightButton" then
		if self.clicked then
			local a,b,c,d,e = self.check:GetPoint(1)
			self.check:SetPoint(a,b,c,d-1,e+1)
			self.clicked = false
		end
	end
end
button_OnMouseUp = wrap(button_OnMouseUp, "button_OnMouseUp")
local function button_OnMouseDown(self, arg1, ...)
	if self.self:HasScript("OnMouseDown") and type(self.self:GetScript("OnMouseDown")) == "function" then
		self.self:GetScript("OnMouseDown")(self.self, arg1, ...)
	end
...
local function button_OnDragStart(self, ...)
	local parent = self:GetParent() and self:GetParent().tablet
...
local function AcquireFrame(self, registration, data, detachedData)
	...
	if tooltip then
		...
		tooltip:SetScript("OnMouseWheel", function(self, arg1)
		...
		slider:SetScript("OnValueChanged", function(self)
			local max = self.tablet.scrollChild:GetHeight() - self.tablet:GetHeight()
			
			local val = self:GetValue() * max
			
			if math.abs(self.tablet.scrollFrame:GetVerticalScroll() - val) < 1 then
				return
			end
			
			self.tablet.scrollFrame:SetVerticalScroll(val)
		end)
...
do
	...
	function StartCheckingAlt(func)
		if not frame then
			...
			frame:SetScript("OnEvent", function(self, _, modifier)
...
function AcquireDetachedFrame(self, registration, data, detachedData)
	...
	detached:SetScript("OnDragStart", function(self)
		detached:StartMoving()
		detached.moving = true
	end)

	detached:SetScript("OnDragStop", function(self)
	...
	detached:SetScript("OnMouseWheel", function(self, arg1)
